...
module.exports = function (pool, app) {
    ...
    app.get('/api/title', middleware.authenticateToken, async (req, res) => {
    try {
        console.time("Title");
        const { title, sortOrder, sortBy } = req.query;
        ...
        // Iterate throuhg each movie
        // Create object to hold all info (star, genres, movie info)
        for ( let i=0; i < result.rows.length; ++i ) {
            let movieObj = {};
